            ...
            if newLastIndex < 0 then
                ...
                let maxChildIndex =
                    if (if leftChildIndex = -1 then 0
                        else data.[leftChildIndex]) > (if rightChildIndex = -1 then 0
                                                       else data.[rightChildIndex])
                    then leftChildIndex
...
let makeDivs (n: int) =
    ...
    let rec loop (i: int) =
        if i * i > n then
            ...
            if n % i = 0 then
                divs.Add(i)
                if i <> 1 && i * i <> n then divs.Add(n / i)
            loop (i + 1)
    loop 1
    divs.Sort()
    divs.ToArray()
...
let makeDivs64 (n: int64) =
    ...
    let rec loop (i: int64) =
        if i * i > int64 n then
            ...
            if n % i = int64 0 then
                divs.Add(i)
                if i <> int64 1 && i * i <> n then divs.Add(n / i)
            loop (i + int64 1)
    loop (int64 1)
    divs.Sort()
    divs.ToArray()
...
let isPrime64 (n: int64) =
    if n = int64 1 then
        ...
        let rec loop (i: int64) =
            if i > n then true
            else if n % i = int64 0 then false
            else loop (i + int64 1)
